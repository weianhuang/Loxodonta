...
public abstract class SimpleObstacle extends Obstacle {
	/* Used to represent z position data */
	private static final int GROUND_CATEGORY_BITS = 0x0001;
	private static final int AIR_CATEGORY_BITS = 0x0002;
	private static final int HURTBOX_CATEGORY_BITS = 0x0004;
	private static final int ENEMY_CATEGORY_BITS = 0x0008;

	public static short getBits(int ground, int air, int hurtbox, int enemy) {
		return (short)((ground>0?GROUND_CATEGORY_BITS:0) + (air>0?AIR_CATEGORY_BITS:0) + (hurtbox>0?HURTBOX_CATEGORY_BITS:0)+ (enemy>0?ENEMY_CATEGORY_BITS:0));
	}

	public void setCategoryBits(int ground, int air, int hurtbox){
		setCategoryBits(ground, air, hurtbox, 0);
	}
	...
	public void setMaskBits(int ground, int air, int hurtbox){
		setMaskBits(ground, air, hurtbox, 1);
	}
	...
	public void setMassData(MassData m){
		body.setMassData(m);
	}
	/**
	...
	public void draw(GameCanvas canvas, Color c) {
		if (texture != null) {
			canvas.draw(texture, c,origin.x,origin.y,getX()*drawScale.x,getY()*drawScale.y,(float)Math.toDegrees(getAngle()),1,1);
		}
	}
