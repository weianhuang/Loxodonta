...
public class LoadingMode implements Screen, InputProcessor {
	...
	private static final String BACKGROUND_FILE = "ui/ui_screens/main_menu_bkg.png";
	private static final String PROGRESS_FILE = "images/progressbar.png";
	private static final String PLAY_BTN_FILE = "ui/ui_buttons/play_button_white.png";
	private static final String SET_BTN_FILE = "ui/ui_buttons/settings_button_white.png";
	private static final String ABOUT_BTN_FILE = "ui/ui_buttons/about_button_white.png";
	private static final String PLAY_BTN_HOVER_FILE = "ui/ui_buttons/play_button_hover.png";
	private static final String SET_BTN_HOVER_FILE = "ui/ui_buttons/settings_button_hover.png";
	private static final String ABOUT_BTN_HOVER_FILE = "ui/ui_buttons/about_button_hover.png";
	...
	/** Amount to offset the about button in the X direction */
	private int ABOUT_BTN_X_OFFSET = 1210;
	/** Amount to offset the about button in the Y direction */
	private int ABOUT_BTN_Y_OFFSET = 630;
	/** Amount to scale the about button */
	private static float ABOUT_BUTTON_SCALE = 0.5f;
	 ...
	 * 0 nothing, 1 hover, 2 pressed, 3 up
	...
	public boolean isReady() {
		if (playButton == null){
			...
			return playButton.get_state() == Button.state.UP;
		}
	}
	...
	private void update(float delta) {
		if (playButton == null) {
			...
			if (progress >= 1.0f) {
				this.progress = 1.0f;
				playButton = new Button(new Texture(PLAY_BTN_FILE), new Texture (PLAY_BTN_HOVER_FILE));
				settingsButton = new Texture(SET_BTN_FILE);
				aboutButton = new Button(new Texture(ABOUT_BTN_FILE), new Texture(ABOUT_BTN_HOVER_FILE));
				...
				aboutButton.setFilter(TextureFilter.Linear, TextureFilter.Linear);
			}
		}
	}
	...
	private void draw() {
		...
		if (playButton == null) {
			...
			//Color tint = (playpressState == 1 ? Color.PURPLE: Color.WHITE);
			canvas.draw(playButton.get_texture(), Color.WHITE, playButton.get_texture().getWidth()/2, playButton.get_texture().getHeight()/2,
				centerX, centerY, 0, BUTTON_SCALE*scale, BUTTON_SCALE*scale);
			float ox = aboutButton.get_texture().getWidth() / 2;
			float oy = aboutButton.get_texture().getHeight() / 2;
			canvas.draw(aboutButton.get_texture(), Color.WHITE, ox, oy, ABOUT_BTN_X_OFFSET, ABOUT_BTN_Y_OFFSET, 0,
					ABOUT_BUTTON_SCALE * scale, ABOUT_BUTTON_SCALE * scale);
		}
		canvas.end();
	}
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		if (playButton == null || playButton.get_state() == Button.state.UP) {
			return true;
		}
		...
		// TODO: Fix scaling
		// About button
		float abouth = ABOUT_BUTTON_SCALE*scale*aboutButton.get_texture().getHeight()/2;
		float aboutw = ABOUT_BUTTON_SCALE*scale*aboutButton.get_texture().getWidth()/2;

		float yOffsetAbout = ABOUT_BTN_Y_OFFSET;
		float xOffsetAbout = ABOUT_BTN_X_OFFSET;

		// Play button
		float sideh = BUTTON_SCALE*scale*playButton.get_texture().getHeight()/2;
		float sidel = BUTTON_SCALE*scale*playButton.get_texture().getWidth()/2;
		if (screenY > centerY - sideh && screenY < centerY + sideh && screenX > centerX - sidel
		&& screenX < centerX + sidel) {
			playButton.set_state(Button.state.DOWN);
		}
		else if (screenY > yOffsetAbout - abouth && screenY < yOffsetAbout + abouth &&
		screenX > xOffsetAbout - aboutw && screenX < xOffsetAbout + aboutw) {
			System.out.println("ABOUT DOWN");
			aboutButton.set_state(Button.state.DOWN);
		}
		return false;
	}
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		if (playButton.get_state() == Button.state.DOWN) {
			playButton.set_state(Button.state.UP);
			return false;
		}
		else if (aboutButton.get_state() == Button.state.DOWN) {
			aboutButton.set_state(Button.state.UP);
			listener.exitScreen(this, 3); // There's probably a better way
			return false;
		}
		return true;
	}
	...
	/*public boolean buttonDown (Controller controller, int buttonCode) {
		if (buttonCode == startButton && playButton.get_state() == Button.state.WAIT) {
			playButton.set_state(Button.state.DOWN);
			return false;
		}
		return true;
...
/*	public boolean buttonUp (Controller controller, int buttonCode) {
		if (playButton.get_state() == Button.state.DOWN && buttonCode == startButton) {
			playButton.set_state(Button.state.UP);
			return false;
		}
		return true;
	...
	public boolean keyUp(int keycode) {
		if (progress == 1){
			...
			if (keycode == Input.Keys.N || keycode == Input.Keys.P || keycode == Input.Keys.SPACE) {
				playButton.set_state(Button.state.UP);
				return false;
			}
		}
	...
	public boolean mouseMoved(int screenX, int screenY) {
		if (progress == 1.0) {
			...
			if (screenY > centerY - sideh/2 && screenY < centerY + sideh/2 && screenX > centerX - sidel/2
					&& screenX < centerX + sidel/2) {
				playButton.set_state(Button.state.HOVER);
			}
			else if (screenY > yOffsetAbout - abouth/2 && screenY < yOffsetAbout + abouth/2 &&
			screenX > xOffsetAbout - aboutw/2 && screenX < xOffsetAbout + aboutw/2){
				aboutButton.set_state(Button.state.HOVER);
			}
			else{
				playButton.set_state(Button.state.WAIT);
				aboutButton.set_state(Button.state.WAIT); 
			}
		}
