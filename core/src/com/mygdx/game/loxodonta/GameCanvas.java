 ...
 * any specific drawing code in PlayMode. All of that code goes here.  As
...
import com.mygdx.game.util.FullScreenShader;
...
public class GameCanvas {
	...
	private FrameBuffer altFrameBuffer;
	private TextureRegion fullScreenTextureRegion;
	 ...
	 */
	 public void resize() {
		...
		altFrameBuffer = new FrameBuffer(Pixmap.Format.RGBA8888, Gdx.graphics.getWidth(), Gdx.graphics.getHeight(), true);
		fullScreenTextureRegion = new TextureRegion(frameBuffer.getColorBufferTexture(), frameBuffer.getWidth(), frameBuffer.getHeight());
		/*fullScreenTextureRegion.setTexture();
		fullScreenTextureRegion.setRegionHeight();
		fullScreenTextureRegion.setRegionWidth();*/
		fullScreenTextureRegion.flip(false, true);
	}
    ...
    public void renderFullscreenShader(FullScreenShader shader) {
		spriteBatch.end();
		frameBuffer.end();
		altFrameBuffer.bind(); // alternate framebuffers so we don't read+write to a single texture at the same time
		spriteBatch.begin();
		spriteBatch.setShader(shader.getShader());
		shader.setUniforms(this);
		fullScreenTextureRegion.setTexture(frameBuffer.getColorBufferTexture());
		//System.out.println("Reading from "+fullScreenTextureRegion.getTexture()+" writing to "+altFrameBuffer.getColorBufferTexture());
		spriteBatch.draw(fullScreenTextureRegion, 0,0, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
		spriteBatch.setShader(null);
		FrameBuffer holder = frameBuffer;
		frameBuffer = altFrameBuffer;
		altFrameBuffer = holder;
		spriteBatch.end();
		spriteBatch.begin();
	}
	...
	public void draw(TextureRegion region, Color tint, float ox, float oy,
					 float x, float y, float angle, float sx, float sy) {
		draw(region, tint, ox, oy, x, y, angle, sx, sy, true);
	}
	 ...
	 */	
	public void draw(TextureRegion region, Color tint, float ox, float oy,
                     float x, float y, float angle, float sx, float sy, boolean useTransform) {
		if (active != DrawPass.STANDARD) {
			Gdx.app.error("GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
			return;
		}
		...
		if (useTransform) local.preMul(transform);
		spriteBatch.setColor(tint);
	...
	public void drawPhysics(Shape shape, Color color, float x, float y, float angle, float sx, float sy) {
		if (shape instanceof PolygonShape){
			drawPhysics((PolygonShape)shape, color, x, y, angle, sx,sy);
		}else if (shape instanceof CircleShape){
			drawPhysics((CircleShape) shape, color, x, y, sx,sy);
		}
	}
	...
	public void setTransform(Affine2 t) {
		transform.set(t);
	}
	...
	public void setTransformXY(float x, float y) {
		transform.setToTranslation(x, y);
	}

	public void setTransformFocus(Vector2 pixelPos, int viewWidth, int viewHeight) {
		int screenHeight = getHeight();
		int screenWidth = getWidth();
		int panHeight = Math.max(0, (viewHeight - getHeight()) / 2);
		int panWidth = Math.max(0, (viewWidth - getWidth()) / 2);
		int offX = (int)pixelPos.x - screenWidth / 2;
		int offY = (int)pixelPos.y - screenHeight / 2;
		if (Math.abs(offX) > panWidth) offX = panWidth * (offX > 0 ? 1 : -1);
		if (Math.abs(offY) > panHeight) offY = panHeight * (offY > 0 ? 1 : -1);
		transform.setToTranslation(-offX, -offY);
	}
}